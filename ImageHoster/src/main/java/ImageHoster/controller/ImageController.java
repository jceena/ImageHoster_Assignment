...
public class ImageController {
    ...
    public String showImage(@PathVariable("title") String title, Model model, @PathVariable String imageId) {
    ...
    @RequestMapping(value = "/image/{imageId}/{imageTitle}/comments", method = RequestMethod.PUT)
    public String comment(@RequestParam("imageId") Integer imageId, @RequestParam("imageTitle") String imageTitle, Image img, Model model){
        Image image = imageService.getImage(imageId);
        img.setImageFile(image.getImageFile());
    ...
    public String editImageSubmit(@RequestParam("file") MultipartFile file, @RequestParam("imageId") Integer imageId, @RequestParam("tags") String tags, Image updatedImage, HttpSession session, Model model) throws IOException {
        ...
        Image img = new Image();
        img = imageService.getImage(imageId);
        User user = (User) session.getAttribute("loggeduser");
        if(user.getId().equals(img.getUser())) {
            Image image = imageService.getImage(imageId);
    ...
    public String deleteImageSubmit(@RequestParam(name = "imageId") Integer imageId, HttpSession session, Model model) {
       ...
       // img.setId(imageId);
        ...
        User user = (User) session.getAttribute("loggeduser");
        if(user.getId().equals(img.getUser().getId())) {
